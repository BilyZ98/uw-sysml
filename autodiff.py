...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        # return [output_grad.dot(B_T), A_T.dot(output_grad)]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        if node in node_to_output_grads_list:
            ...
            sum_grad =  sum_node_list(node_to_output_grads_list[node])
            node_to_output_grad[node] = sum_grad
            # node_to_output_grad[node] = sum_node_list(node_to_output_grads_list[node])
            grads = node.op.gradient(node, sum_grad)

            # if node_grad is not None:
            #     print('type node.inputs', type(node.inputs), 'node name', node.name,  'type node_grad', type(node_grad))

            for id, in_node in enumerate(node.inputs):
                if in_node not in node_to_output_grads_list:
                    node_to_output_grads_list[in_node] = []
                node_to_output_grads_list[in_node].append(grads[id])

            # This is wrong becuse we might have constant as node and gradient for constant is None
            # for input_node, input_grad in zip(node.inputs, grads):
...
def test():
    ...
    # print(executor.run(feed_dict={x2:7}))
